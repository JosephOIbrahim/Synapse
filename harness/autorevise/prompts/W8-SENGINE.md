# W8-SENGINE â€” engine scout: five-backend resilience, routing, offline, injection defense

You are a SYNAPSE wave agent on branch `wave8/sengine` in worktree `.claude/worktrees/w8-sengine`.
...
```json
{
  "id": "W8-SENGINE",
  "band": "TRUTH",
  "name": "engine scout: five-backend resilience, routing, offline, injection defense",
  "source": {
  ...
  "targets": [
    ...
    "5) Output: receipt harness/notes/receipts/W8-SENGINE.json + findings ranked P0 (production-blocking) / P1 (hardening) / P2 (polish), each with a file:line anchor and first-hand observation or UNKNOWN.",
...
1. **Before touching any file in `touches`** â€” post a claim:
   `python C:\Users\User\SYNAPSE\harness\autorevise\bus.py post wave8 W8-SENGINE claim '{\"files\": [\"<paths>\"]}'`
...
2. **Findings** as you go:
   `python C:\Users\User\SYNAPSE\harness\autorevise\bus.py post wave8 W8-SENGINE finding '{\"claim\": \"...\", \"anchor\": \"file:line\"}'`
3. **Release** when done editing:
   `python C:\Users\User\SYNAPSE\harness\autorevise\bus.py post wave8 W8-SENGINE status '{\"release\": [\"<same paths>\"]}'`
4. **Read before you act** on any shared seam:
   `python C:\Users\User\SYNAPSE\harness\autorevise\bus.py read wave8 W8-SENGINE`
...
Write `harness/notes/receipts/W8-SENGINE.json` **inside your worktree**:
`{{"leg": "W8-SENGINE", "status": "green|green_with_findings|blocked",
