# W8-SSHIELD â€” shield scout: secrets, deps, telemetry, patents-pending repo hygiene

You are a SYNAPSE wave agent on branch `wave8/sshield` in worktree `.claude/worktrees/w8-sshield`.
...
```json
{
  "id": "W8-SSHIELD",
  "band": "TRUTH",
  "name": "shield scout: secrets, deps, telemetry, patents-pending repo hygiene",
  "source": {
  ...
  "targets": [
    ...
    "5) Output: receipt harness/notes/receipts/W8-SSHIELD.json + findings ranked P0 (production-blocking) / P1 (hardening) / P2 (polish), each with a file:line anchor and first-hand observation or UNKNOWN.",
...
1. **Before touching any file in `touches`** â€” post a claim:
   `python C:\Users\User\SYNAPSE\harness\autorevise\bus.py post wave8 W8-SSHIELD claim '{\"files\": [\"<paths>\"]}'`
...
2. **Findings** as you go:
   `python C:\Users\User\SYNAPSE\harness\autorevise\bus.py post wave8 W8-SSHIELD finding '{\"claim\": \"...\", \"anchor\": \"file:line\"}'`
3. **Release** when done editing:
   `python C:\Users\User\SYNAPSE\harness\autorevise\bus.py post wave8 W8-SSHIELD status '{\"release\": [\"<same paths>\"]}'`
4. **Read before you act** on any shared seam:
   `python C:\Users\User\SYNAPSE\harness\autorevise\bus.py read wave8 W8-SSHIELD`
...
Write `harness/notes/receipts/W8-SSHIELD.json` **inside your worktree**:
`{{"leg": "W8-SSHIELD", "status": "green|green_with_findings|blocked",
