# W8-SSURF â€” surface scout: install, first-run, UX debt, error language, operator docs

You are a SYNAPSE wave agent on branch `wave8/ssurf` in worktree `.claude/worktrees/w8-ssurf`.
...
```json
{
  "id": "W8-SSURF",
  "band": "TRUTH",
  "name": "surface scout: install, first-run, UX debt, error language, operator docs",
  "source": {
  ...
  "targets": [
    ...
    "5) Output: receipt harness/notes/receipts/W8-SSURF.json + findings ranked P0 (production-blocking) / P1 (hardening) / P2 (polish), each with a file:line anchor and first-hand observation or UNKNOWN.",
...
1. **Before touching any file in `touches`** â€” post a claim:
   `python C:\Users\User\SYNAPSE\harness\autorevise\bus.py post wave8 W8-SSURF claim '{\"files\": [\"<paths>\"]}'`
...
2. **Findings** as you go:
   `python C:\Users\User\SYNAPSE\harness\autorevise\bus.py post wave8 W8-SSURF finding '{\"claim\": \"...\", \"anchor\": \"file:line\"}'`
3. **Release** when done editing:
   `python C:\Users\User\SYNAPSE\harness\autorevise\bus.py post wave8 W8-SSURF status '{\"release\": [\"<same paths>\"]}'`
4. **Read before you act** on any shared seam:
   `python C:\Users\User\SYNAPSE\harness\autorevise\bus.py read wave8 W8-SSURF`
...
Write `harness/notes/receipts/W8-SSURF.json` **inside your worktree**:
`{{"leg": "W8-SSURF", "status": "green|green_with_findings|blocked",
