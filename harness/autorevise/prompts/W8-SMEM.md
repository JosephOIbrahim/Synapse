# W8-SMEM â€” memory scout: Moneta hardening, capsules, SQLite+FTS5 readiness

You are a SYNAPSE wave agent on branch `wave8/smem` in worktree `.claude/worktrees/w8-smem`.
...
```json
{
  "id": "W8-SMEM",
  "band": "TRUTH",
  "name": "memory scout: Moneta hardening, capsules, SQLite+FTS5 readiness",
  "source": {
  ...
  "targets": [
    ...
    "5) Output: receipt harness/notes/receipts/W8-SMEM.json + findings ranked P0 (production-blocking) / P1 (hardening) / P2 (polish), each with a file:line anchor and first-hand observation or UNKNOWN.",
...
1. **Before touching any file in `touches`** â€” post a claim:
   `python C:\Users\User\SYNAPSE\harness\autorevise\bus.py post wave8 W8-SMEM claim '{\"files\": [\"<paths>\"]}'`
...
2. **Findings** as you go:
   `python C:\Users\User\SYNAPSE\harness\autorevise\bus.py post wave8 W8-SMEM finding '{\"claim\": \"...\", \"anchor\": \"file:line\"}'`
3. **Release** when done editing:
   `python C:\Users\User\SYNAPSE\harness\autorevise\bus.py post wave8 W8-SMEM status '{\"release\": [\"<same paths>\"]}'`
4. **Read before you act** on any shared seam:
   `python C:\Users\User\SYNAPSE\harness\autorevise\bus.py read wave8 W8-SMEM`
...
Write `harness/notes/receipts/W8-SMEM.json` **inside your worktree**:
`{{"leg": "W8-SMEM", "status": "green|green_with_findings|blocked",
