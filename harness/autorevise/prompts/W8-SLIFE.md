# W8-SLIFE â€” lifecycle scout: runtime/session lifetime vs UI, undo, threads, recovery

You are a SYNAPSE wave agent on branch `wave8/slife` in worktree `.claude/worktrees/w8-slife`.
...
```json
{
  "id": "W8-SLIFE",
  "band": "TRUTH",
  "name": "lifecycle scout: runtime/session lifetime vs UI, undo, threads, recovery",
  "source": {
  ...
  "targets": [
    ...
    "5) Output: receipt harness/notes/receipts/W8-SLIFE.json + findings ranked P0 (production-blocking) / P1 (hardening) / P2 (polish), each with a file:line anchor and first-hand observation or UNKNOWN.",
...
1. **Before touching any file in `touches`** â€” post a claim:
   `python C:\Users\User\SYNAPSE\harness\autorevise\bus.py post wave8 W8-SLIFE claim '{\"files\": [\"<paths>\"]}'`
...
2. **Findings** as you go:
   `python C:\Users\User\SYNAPSE\harness\autorevise\bus.py post wave8 W8-SLIFE finding '{\"claim\": \"...\", \"anchor\": \"file:line\"}'`
3. **Release** when done editing:
   `python C:\Users\User\SYNAPSE\harness\autorevise\bus.py post wave8 W8-SLIFE status '{\"release\": [\"<same paths>\"]}'`
4. **Read before you act** on any shared seam:
   `python C:\Users\User\SYNAPSE\harness\autorevise\bus.py read wave8 W8-SLIFE`
...
Write `harness/notes/receipts/W8-SLIFE.json` **inside your worktree**:
`{{"leg": "W8-SLIFE", "status": "green|green_with_findings|blocked",
