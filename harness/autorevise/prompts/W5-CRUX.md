...
```json
{
  ...
  "targets": [
    ...
    "6) CROSS-REFERENCE AUDIT: probe the merged retrieval with anatomy-derived questions from rag/skills/houdini21-reference/solaris_compound_node_anatomy.md (karma material builder, componentgeometry outputs incl. H22 'alternative', instancer type resolution, the three user-editable island paths) - any answer contradicting the live-verified doc is a finding; verify the BUS CONTRACT was honored: DELTA posted census+contradictions, DENSE posted index stamp and demonstrably consumed DELTA's findings before its final eval (bus timestamps are the evidence)"
  ],
  "acceptance": [
    {
    ...
    {
      "predicate": "bus audit: DELTA and DENSE contract findings exist with coherent timestamps; anatomy cross-reference probes recorded with zero unresolved contradictions or each contradiction filed as a finding",
      "evidence": "check"
    }
  ],
  "deps": [
