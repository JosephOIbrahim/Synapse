# mission_schema.py - APEXFORGE mission validation (cloned from autorevise 2026-08-17). Plain Python, zero deps.
...
ID_RE = re.compile(r"^WA\d+-[A-Z0-9]{2,12}$")
...
def validate_mission(m: dict) -> list:
    ...
    if not ID_RE.match(m["id"]):
        _err(errors, mid, "id must match WA<wave>-<TAG> e.g. WA1-TRUTH (apexforge family)")
