# compile_wave.py - APEXFORGE missions -> legs/v1 rows + prompt briefs (cloned from autorevise).
...
def leg_row(m: dict) -> dict:
    ...
    return {
        ...
        "prompt": f"harness/apexforge/prompts/{m['id']}.md",
        "deps": m["deps"],
        "readonly": m["readonly"],
        "touches": m["touches"],
        "note": f"APEXFORGE {m['source']['doc']} :: {m['source']['anchor']}. {m.get('note','')}".strip(),
    }
