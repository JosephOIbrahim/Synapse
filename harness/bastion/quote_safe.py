"""quote_safe.py - BASTION harness v2 central quoting + encoding helpers (python
twin of harness/lib/quote-safe.ps1). FORK of harness/autorevise/quote_safe.py
(traced 2026-08-17, W8-SMITH), verbatim - this is the debom discipline the fork
must preserve (survival rule 5).
...
The bastion python surface, like the autorevise one, is clean by construction (no
shell subprocess calls; every json.dumps uses encoding="utf-8", which is BOM-free),
so these are provided as the shared helper + the test oracle rather than
retrofitted over a broken call site. Use them for any NEW python that builds a
PowerShell command or writes JSON.
"""
