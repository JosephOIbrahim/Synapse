...
- **release arm (orthogonal to A/B):** a human flips `R.0`'s `ratified` in the git-tracked
  `harness/state/flywheel_queue.json` (the D.0 JSON-predicate pattern) → `R.1`–`R.10` + `R.R`
  (`blocked_on:"release_ratified"`) arm: the 2026-07-10 external H22 readiness review as a gate
  wall (12 fingerprint gates, all RED now = accurate). R.9 is human_gate (security). R.R needs
  human receipts (`release_receipts.json`) + Mode-B host truth to label STABLE — un-bankable
  before the drop, by design. Verdict computable unratified via `checks.py --task R.R`.
  Contract: `harness/notes/spec-R-release-readiness.md`.
...
- 2026-07-10 · release track grafted (additive): R.0 ratification trigger (second JSON-predicate trigger, D.0 pattern) + receipts file `harness/state/release_receipts.json` (human-authored live-gate attestation, peer of posture.json). Wraps docs/reviews/synapse-h22-readiness-2026-07-10.md (external CTO review, "H22 RC0") into 12 gates: R.1–R.6 = P0.1–P0.6 in the review's Phase-1 order, R.7/R.8 P1 hygiene-with-teeth, R.9 human_gate security (closes the shared/bridge.py get_process_bridge disarm S.2 is BLIND to + the wholly-ungated auth surface), R.10 packaging (post-stabilization, non-blocking), R.R capstone → release_readiness_verdict.json with the G1–G10 map (STABLE-READY only on machine-green + receipts + host truth + G10 claim-vs-receipts). Every review claim adversarially verified first (11-agent sweep, all CONFIRMED; sweep found a THIRD fail-open site mcp/tools.py:119 and the panel-close FALSE-POSITIVE freeze escalation). All 11 non-capstone gates RED on the live tree = accurate; pinned by tests/test_r_track.py incl. a live-tree RED-honesty case. Cycle R.0 queued ratified:false (anti-runaway anchor — the flip is yours).
...
- 2026-07-16 · DROP-WEEK §9 STEPS 1–9 COMPLETE (h22-relay → h22-drop-week workflow, MODE B, H22.0.368 via HYTHON; bridge down → all probe verdicts PROVISIONAL-headless). Artifacts: h22-abi-verdict (gate-0.1 re-opened, re-vendor already executed @38b33b6), h22-quarantine-repin (4/4 re-pinned QUARANTINE; hou.secure NOT adopted, live-bridge reconfirm owed), verified_connectivity_22.0.368.json + _H22 diff (+9/−3/3Δ; instancer/layout LOP removals confirmed; U.1-H22 candidate deposited ratified:false), COP audit (21 tools: 11 PASS/10 CHANGED/0 GONE; COP2 legacy survives), h22-qt-smoke PASS (run_panel.py stub fix: isUIAvailable), release-notes adjudication (C2 CLOSED-with-rider: APEX Script Comfort Package MCP announced-not-shipped; rigging boundary pressure ×3 REJECT), pdg perception re-audit (4/4 H21 truths hold; 2 pdg.Scheduler removals, 0 repo hits), step-9 probes (hou.imageResolution PASS; top::gaussian_splat_train PHANTOM → real name ml_traingsplats). Suite 4278/0/87 (floor 4275 held). Harness fix: .claude/agents/assayer.md frontmatter had unquoted ": " → invalid YAML → agent silently never registered (registry is also session-frozen; workflow child errors memoize by name+args — bust with a throwaway arg). STEP 10 (ratify) + Solaris layout ruling + merge = human.
