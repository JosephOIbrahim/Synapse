...
**17:0x addendum:** INS forge observed building in MAIN TREE (handlers_cache.py +93/-12, correct content, wrong location - mission wording 'extend it' pointed at the main-tree path, my miss). Closer v1 killed; CLOSER V2 relaunched (new PID in receipt log): ALL-OR-NOTHING - both reviewers must be CLEAR or nothing merges and nothing pushes; dirty-tree guard retained. Session 3: if V2 held everything local, reconcile the main-tree insert edits onto feat/cache-insert (or accept-on-master with explicit ruling) BEFORE any push. Capsule committed+pushed at 18b38584.
