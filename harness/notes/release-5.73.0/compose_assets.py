...
Run from the repo root after build + qualification + suites:
    ...
    python harness/notes/release-5.73.0/compose_assets.py [--ci-run-id N]

Writes, into C:/synapse-build/output-5.73.0/output/:
    SHA256SUMS.txt
...
BUILD = r"C:/synapse-build/output-5.73.0"
OUT = os.path.join(BUILD, "output")
NOTES = os.path.join(REPO, "harness/notes/release-5.73.0")
...
report = {
    ...
    "local_stock_suite": {"summary": STOCK, "scope": "Local Windows, stock Python 3.14, vendored SDK inactive. Run alone, with ANTHROPIC_* unset -- the SDK reads ANTHROPIC_AUTH_TOKEN from the environment and the guarded-lane check then refuses the client it was just handed."},
