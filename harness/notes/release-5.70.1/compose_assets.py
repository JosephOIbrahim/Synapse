...
Run from the repo root after build + qualification + suites:
    ...
    python harness/notes/release-5.70.1/compose_assets.py [--ci-run-id N]

Writes, into C:/synapse-build/output-5.70.1/output/:
    SHA256SUMS.txt
...
BUILD = r"C:/synapse-build/output-5.70.1"
OUT = os.path.join(BUILD, "output")
NOTES = os.path.join(REPO, "harness/notes/release-5.70.1")
...
report = {
    ...
    "payload_audit": {
        ...
        "product_change_since_v5.70.0": {
            ...
            "result": subprocess.run(["git", "-C", REPO, "diff", "--stat", "v5.70.0", REV, "--", "python", "installer"],
