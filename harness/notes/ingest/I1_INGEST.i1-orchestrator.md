...
the new COPs       3     ->  168     ingested, at the floor, live-type-matched
                                   (and the governing "161" is itself wrong -- see §1)
```
...
## 1 · The new Copernicus nodes — and the governing number is wrong
...
> **CORRECTION, and it invalidates a figure this leg inherited.** The shipped page links
> node paths in **two** forms — `Node:/cop/name` (161 occurrences) and `Node:cop/name` (11).
> A pattern requiring the leading slash returns **161** and silently drops **10 real node
> types**: the entire `adjacency_*` family, `layerattribcreate`/`layerattribdelete`,
> `geotoadjacency`, and three `block_begin`/`block_end` nodes. **All 10 have a page and all
> 10 clear the floor.** The number is **171**. `161` is a floor, not a total.
>
> It is wrong in `docs/H22_FRONTIER.md`, in I0's Q3 (*"161 named, 161 present, 0 absent"* —
> from two sources that agreed *because both made the same assumption*), in
> `harness/SYNAPSE_INGEST.md`, in this leg's own brief, **and in this extractor's first
> version, whose POSITIVE control asserted 161 and therefore PINNED the defect.**
> A control whose expectation is inherited from the brief rather than measured from the
> archive locks in the brief's error (Law 5). Found by the concurrent I1 run
> (`.claude/remediation_ticket.md`) and verified here independently before being accepted.

```
named in the shipped what's-new      171      <- both link forms; was reported as 161
have a cop/<name>.txt                171      0 absent
INGESTED (clear the floor)           168      98.2%
known-thin                             3
...
```
i1_calibrate.py     35/35 controls pass
  POSITIVE  19   pages read BY HAND whose exact answer the parser must reproduce
  NEGATIVE   7   mutations that must drive a count to zero or flip a verdict
  BLIND      8   deliberately naive readers, shown returning the WRONG answer
  CROSS-CHECK 1  an archive fact I0 measured independently (nodes.zip BOM count = 58)

Two of those controls exist only because this leg's first version got the 161 wrong:
**B8** shows a leading-slash-only link pattern returning 161 and dropping 10 types, and
**P6b** proves all 10 have pages that clear the floor. Neither existed when the number was
