# watch_w5l_verdict.ps1 - detached sentinel. Polls for the W5-LCRUX verdict;
...
while ((Get-Date) -lt $deadline) {
    $c = @(Get-ChildItem 'C:\Users\User\SYNAPSE\.claude\worktrees\w5-*\harness\notes\receipts\W5-LCRUX.json','C:\Users\User\SYNAPSE\harness\notes\receipts\W5-LCRUX.json' -ErrorAction SilentlyContinue)
    if ($c.Count -gt 0) {
        $t = Get-Date -Format 'HH:mm:ss'
        "LANDED $t  $($c[0].FullName)" | Out-File $flag -Encoding ascii
        [System.Windows.Forms.MessageBox]::Show(
            "W5-LCRUX verdict landed at $t.`n`n$($c[0].FullName)`n`nRead it, then give the merge words - per leg.",
            "SYNAPSE - lifecycle wave",
...
"TIMEOUT $(Get-Date -Format 'HH:mm:ss') - no verdict within 10h" | Out-File $flag -Encoding ascii
