...
def _blocking(v: dict) -> tuple[list[str], list[str]]:
    """(failures that refuse the leaf, dead-gate failures waived) from a verifier's own rerun."""
    block, waived = [], []
    for a in v.get("acceptance_rerun", []):
        if a.get("verdict") == "pass":
            continue
        p = " ".join((a.get("predicate") or "").split())
        (waived if any(rx.search(p) for rx in DEAD_GATES) else block).append(p)
    return block, waived
...
def mergeable(leg: str, res: dict) -> tuple[str, str, str, list[str]] | None:
    ...
    if v.get("verdict") in (None, "BROKEN"):
        return None
    block, waived = _blocking(v)
    if block:
        return None
    return (leg, rc.get("branch") or rc["commit_sha"], v.get("verdict", "?"), waived)
...
def plan(res: dict) -> list[tuple[str, str, str]]:
    ...
    for chain in LEAF_CHAINS:
        ...
        if leg not in said:
            said.add(leg)
            for w in waived:
                print(f"   WAIVED  {leg}: dead gate, replaced by panel_gate.py -- {w[:96]}")
        if branch in seen:
            continue
        seen.add(branch)
    ...
    for leg in (l for chain in LEAF_CHAINS for l in chain):
        r = res.get(leg) or {}
        block, _ = _blocking((r.get("verdict") or {}))
        for b in block:
