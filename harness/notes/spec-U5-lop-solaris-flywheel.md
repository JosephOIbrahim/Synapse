...
**Status:** built + shipped to branch `feat/h22-knowledge-flywheel` (commit `fea231d`) · **Layer:** utility · **Mode:** A (runs now on H21.0.671)
...
2. **REVIEW — grounding gate + severity design.** Prose truth has no `setInput(` call-site analog, so two surfaces stand in for U.1's code sweep: (a) the miner's **consistency gate** is the standing review — it fails loud if an authored claim is grounded in neither corpus nor probe; (b) an **adversarial design review** (two independent verification passes) set the error-vs-advisory severity split and caught a hard-error false-positive (see "The severity call"). Ledger deposit of the review findings via `synapse.science.deposit.LedgerDeposit` (as U.1 did) is **OWED**.
...
3. **SCAFFOLD — wire into the live path + pin.**
   ...
   - `harness/verify/checks.py` verbs (e.g. `lop_knowledge_fresh`, `lop_advisory_conformance`, ADAPT-marker style) — **OWED** (not yet added).
...
| `flywheel_queue.json` + `tasks.json` U.5 entry (evidence-linked) | ⚠ OWED |
| Human ratification (`ratified: true`) | ⚠ OWED |
| Ledger deposit of the review findings | ⚠ OWED |
| `harness/verify/checks.py` U.5 verbs | ⚠ OWED |

## Anti-runaway anchors

- **Human ratifies new cycle CLASSES.** U.5 introduced a new truth axis (CONTEXT truth, distinct from U.1–U.4's WIRING truth). Per the standing anchor, a new class needs human ratification **before** building; U.5 was built ahead of that record. A retroactive evidence-linked queue entry + `ratified: true` (human-flipped) is owed to bring it into compliance — this spec is that entry's evidence.
...
Full `python -m pytest tests/ -q` green (modulo pre-existing release-train failures, reported as a delta) AND — once added — `harness/verify/checks.py --task U.5` PASS. Current state: `tests/test_lop_flywheel.py` 16/16 + 160 green across the flywheel + adjacent validator/graph/solaris suites; `scripts/mine_lop_knowledge.py --check` byte-identical (blake2b `685ced35bb371bcc9973215345d126a8`).
