# dashboard_bp4.py - BP4 progress board for docs/BATTLEPLAN.md (2026-09-01).
...
# Writes harness/battleplan/board_bp4.html (auto-refresh 30 s) and prints a text board.
# Reads only; never dispatches, never flips anything. UNKNOWN renders as UNKNOWN.
# Run: python harness/battleplan/dashboard_bp4.py            (once)
#      python harness/battleplan/dashboard_bp4.py --watch    (every 30 s)
#      python harness/battleplan/dashboard_bp4.py --open     (once, then opens the html)
...
VERDICTS = BP / "notes" / "BP4-CRUX_verdicts.md"
OUT = BP / f"board_{WAVE}.html"
UNKNOWN = "UNKNOWN"

PAIRS = {"BP4-INTAKE": "builder", "BP4-RULINGS": "builder", "BP4-B7FIX": "builder", "BP4-SPATIAL": "builder",
...
DEMO = [
    ...
    (6, "Memory latency inside a stated camera budget", "UNKNOWN", "BP4-LATENCY"),
    (7, "Repeat deposits don't corrupt the store (FU-1)", "RED", "BP4-STORE"),
    (8, "Panel opens docked; no float hijack", "RED", "BP4-PANELTRUTH T3 + your eyes"),
    (9, "Profiles visibly differ; switch persists", "UNKNOWN", "BP4-PANELTRUTH T1"),
    (10, "TOKEN readout updates per task", "RED", "BP4-PANELTRUTH T2"),
    (11, "Panel spacing at the Cohere rhythm (camera regions)", "RED", "BP4-PANELDESIGN (held)"),
    (12, "Harness spend measured in tokens, caps enforceable", "RED", "BP4-METER"),
...
def demo_rows(legs, led):
    ...
    for n, item, status, ev in DEMO:
        ...
        for k, leg in ((7, "BP4-STORE"), (9, "BP4-PANELTRUTH"), (10, "BP4-PANELTRUTH")):
...
def words(legs, pid_alive_):
    ...
    return [
        ...
        ("go batch (8 legs, SPATIAL included)", f"orchestrator pid {pid} ALIVE" if alive else ("pid file present, process DEAD" if pid else "not armed - `powershell -File harness\\battleplan\\arm_bp4.ps1`"), not alive),
        ...
        ("rule the BP3 banked items (CTO, cold) -> ratify flips", "after BP4-RULINGS lands harness/notes/CTO_RULINGS_BP3.md", "BP4-RULINGS" in receipted),
        ("prune worktrees (TIDY census, merged AND clean only)", "after BP4-TIDY lands harness/battleplan/notes/BP4_TIDY.md", "BP4-TIDY" in receipted),
        ("GUI hands: paste probe_ui_font.py in Houdini 22.0.400, before/after panel screenshots", "after BP4-PANELFONT lands", "BP4-PANELFONT" in receipted),
    ]
...
def render_html(legs, pidinfo, led, bus_tail, demo, wds, mile, today):
    ...
    h = [f"<!doctype html><html><head><meta charset='utf-8'><meta http-equiv='refresh' content='30'><title>BP4 board</title><style>{CSS}</style></head><body>"]
    h.append(f"<h1>SYNAPSE · Battle Plan <em>BP4</em></h1>")
    ...
    tail = "\n".join(LOG.read_text(encoding="utf-8", errors="replace").splitlines()[-8:]) if LOG.exists() else "no orchestrator-bp4.log yet"
    h.append(f"<div class='log'>pid {e(str(pid))} {'ALIVE' if alive else ('DEAD' if pid else 'not armed')}\n{e(tail)}</div>")
    h.append(f"<div class='foot'>docs/BATTLEPLAN.md 2026-09-01 · harness/battleplan/dashboard_bp4.py · reads only · refresh 30 s</div></body></html>")
    return "\n".join(h)

def render_text(legs, pidinfo, led, bus_tail, demo, wds, mile, today):
    pid, alive = pidinfo
    lines = [f"== BP4 board - demo week mile {mile} of 8 - {today} - {datetime.now().strftime('%H:%M:%S')} ==",
