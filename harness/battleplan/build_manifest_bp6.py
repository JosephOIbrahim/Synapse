# build_manifest_bp6.py - BP6 rows -> waves/bp6.live.json (legs/v1) for orchestrate.ps1.
# Clone of build_manifest_bp4.py (wave id swap). BP6 = one leg, BP6-JEV, the first mission
# routed by JEV-ROUTE (tier:auto -> resolved at compile; see harness/jev/ledger/bp6.route.jsonl).
...
rows = json.loads((AF / "waves" / "bp6.rows.json").read_text(encoding="utf-8"))
for r in rows:
    if r["id"] in HELD:
        r["state"] = "held"
...
man = {
    "_comment": ('BP6 BATTLEPLAN wave - one leg (JEV) compiled from harness/battleplan/notes/JEV_BLUEPRINT.md. '
                 f'Tier resolved by JEV-ROUTE at compile (ledger bp6.route.jsonl). Base=master {base}. ARMED only on '
                 'Joe word, WITH -Budget (rails turn = leg dispatch). Merge/push remain Joe words per act. '
                 'Own bus (battleplan/bus/bp6), own worktree prefix (bp6-*).'),
    "_schema": "legs/v1",
