# arm_bp3.ps1 - arm wave BP3 (clone of arm_bp1.ps1, wave id swap) WITH budget rails.
...
# re-dispatch/spawn. Tokens stay UNKNOWN until BP3-METER's settle lands.
# ARMED ONLY ON JOE'S WORD. Never headless through DC; DC polls the log.
param([string]$Budget = '12turns')
$ErrorActionPreference = 'Stop'
$repo = 'C:\Users\User\SYNAPSE'
$af   = Join-Path $repo 'harness\battleplan'

# kill a previously-armed bp3 orchestrator if its pid file is live
$pidFile = Join-Path $repo 'harness\notes\h22\orchestrator-bp3.pid'
if (Test-Path $pidFile) {
    $old = Get-Content $pidFile
    $proc = Get-CimInstance Win32_Process -Filter "ProcessId=$old" -ErrorAction SilentlyContinue
    if ($proc) { Stop-Process -Id $old -Force -ErrorAction SilentlyContinue; Write-Host ("killed stale bp3 orchestrator pid " + $old) }
}

python (Join-Path $af 'build_manifest_bp3.py')
if ($LASTEXITCODE -ne 0) { throw 'manifest build failed' }

$env:CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS = '1'
$env:CLAUDE_CODE_PRINT_BG_WAIT_CEILING_MS = '0'
$manPath = Join-Path $af 'waves\bp3.live.json'
$outLog  = Join-Path $repo 'harness\notes\h22\orchestrator-bp3.log'
$errLog  = Join-Path $repo 'harness\notes\h22\orchestrator-bp3.err'
...
Write-Host ("bp3 orchestrator armed, pid " + $p.Id + ", budget " + $Budget + " (rails turn = leg dispatch)")
