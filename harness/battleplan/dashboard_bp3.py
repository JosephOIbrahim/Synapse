# dashboard_bp3.py - BP3 progress board for docs/BATTLEPLAN.md (2026-09-01).
...
# Writes harness/battleplan/board_bp3.html (auto-refresh 30 s) and prints a text board.
# Reads only; never dispatches, never flips anything. UNKNOWN renders as UNKNOWN.
# Run: python harness/battleplan/dashboard_bp3.py            (once)
#      python harness/battleplan/dashboard_bp3.py --watch    (every 30 s)
#      python harness/battleplan/dashboard_bp3.py --open     (once, then opens the html)
...
VERDICTS = BP / "notes" / "BP3-CRUX_verdicts.md"
OUT = BP / f"board_{WAVE}.html"
UNKNOWN = "UNKNOWN"

PAIRS = {"BP3-METER": "pair 1", "BP3-PANELTRUTH": "pair 1", "BP3-LATENCY": "pair 2",
         "BP3-STORE": "pair 2", "BP3-PANELDESIGN": "solo (Wed word)", "BP3-CRUX": "solo (referee)"}
...
DEMO = [
    ...
    (6, "Memory latency inside a stated camera budget", "UNKNOWN", "BP3-LATENCY"),
    (7, "Repeat deposits don't corrupt the store (FU-1)", "RED", "BP3-STORE"),
    (8, "Panel opens docked; no float hijack", "RED", "BP3-PANELTRUTH T3 + your eyes"),
    (9, "Profiles visibly differ; switch persists", "UNKNOWN", "BP3-PANELTRUTH T1"),
    (10, "TOKEN readout updates per task", "RED", "BP3-PANELTRUTH T2"),
    (11, "Panel spacing at the Cohere rhythm (camera regions)", "RED", "BP3-PANELDESIGN (held)"),
    (12, "Harness spend measured in tokens, caps enforceable", "RED", "BP3-METER"),
...
def demo_rows(legs, led):
    ...
    for n, item, status, ev in DEMO:
        ...
        for k, leg in ((7, "BP3-STORE"), (9, "BP3-PANELTRUTH"), (10, "BP3-PANELTRUTH")):
...
def words(legs, pid_alive_):
    ...
    return [
        ...
        ("arm BP3 pairs 1+2", f"orchestrator pid {pid} ALIVE" if alive else ("pid file present, process DEAD" if pid else "not armed - `powershell -File harness\\battleplan\\arm_bp3.ps1`"), not alive),
        ...
        ("Wed: flip BP3-PANELDESIGN held -> ready (build_manifest_bp3.py HELD) and re-arm", "after PANELTRUTH is merged", True),
    ]
...
def render_html(legs, pidinfo, led, bus_tail, demo, wds, mile, today):
    ...
    h = [f"<!doctype html><html><head><meta charset='utf-8'><meta http-equiv='refresh' content='30'><title>BP3 board</title><style>{CSS}</style></head><body>"]
    h.append(f"<h1>SYNAPSE · Battle Plan <em>BP3</em></h1>")
    ...
    tail = "\n".join(LOG.read_text(encoding="utf-8", errors="replace").splitlines()[-8:]) if LOG.exists() else "no orchestrator-bp3.log yet"
    h.append(f"<div class='log'>pid {e(str(pid))} {'ALIVE' if alive else ('DEAD' if pid else 'not armed')}\n{e(tail)}</div>")
    h.append(f"<div class='foot'>docs/BATTLEPLAN.md 2026-09-01 · harness/battleplan/dashboard_bp3.py · reads only · refresh 30 s</div></body></html>")
    return "\n".join(h)

def render_text(legs, pidinfo, led, bus_tail, demo, wds, mile, today):
    pid, alive = pidinfo
    lines = [f"== BP3 board - demo week mile {mile} of 8 - {today} - {datetime.now().strftime('%H:%M:%S')} ==",
