# BP8-CRUX — CRUX: adversarial re-verification of BP8-WATCHDOG and BP8-TIMEOUTS

You are a SYNAPSE BATTLEPLAN wave agent on branch `bp8/crux` in worktree
`.claude/worktrees/bp8-crux`. Model: resolved per mission tier by harness/rails_exec.json (mechanical Haiku 4.5 / reasoning Opus 4.8 / referee Fable 5.1), dispatched by harness/orchestrate.ps1. This
...
```json
{
  ...
  "note": "Tier: referee. Start from the JEV-SCREEN line for each leg (run `python harness/jev/jev_screen.py --wave bp8`): CLEAR = spot-check two rows, REFEREE = full read, FLAG = begin at the flagged row. The screen sets depth only; it never replaces your own mutations. gui_required predicates are UNKNOWN to you - say so.",
  "targets": [
    "T1) Per builder receipt: re-run every non-GUI acceptance predicate in a fresh checkout of the leg branch; verdict rows pass|fail|UNKNOWN with your own anchors.",
    ...
    "T3) harness/battleplan/notes/BP8-CRUX_verdicts.md: one verdict per leg (SOUND | SOUND-WITH-NITS | BROKEN), plus one line comparing your verdict with the JEV-SCREEN line for that leg."
  ],
  "touches": [],
  "acceptance": [
    {
      "predicate": "one verdict per builder leg (two) with independently re-run rows and the crucible's own anchors",
      "evidence": "receipt"
    },
    {
      "predicate": ">= 2 self-authored mutations per builder leg, each named with the check it reddens",
      "evidence": "test"
    },
    {
      "predicate": "screen-vs-CRUX agreement line present for both legs",
      "evidence": "receipt"
    }
  ],
  "crucible_criteria": [
  ...
  "source": {
    ...
    "anchor": "whole verdict; harness/battleplan/notes/JEV_BLUEPRINT.md sec.3.2 (screen line in the CRUX brief)"
  }
}
```
...
1. **Before touching any file in `touches`** — post a claim:
   `python C:\Users\User\SYNAPSE\harness\battleplan\bus.py post bp8 BP8-CRUX claim '{\"files\": [\"<paths>\"]}'`
...
2. **Findings** as you go — and the moment an evidence artifact lands, post its
   ...
   `python C:\Users\User\SYNAPSE\harness\battleplan\bus.py post bp8 BP8-CRUX finding '{\"claim\": \"...\", \"anchor\": \"file:line-or-artifact-path\"}'`
3. **Release** when done editing:
   `python C:\Users\User\SYNAPSE\harness\battleplan\bus.py post bp8 BP8-CRUX status '{\"release\": [\"<same paths>\"]}'`
4. **Read before you act** on any shared seam, and poll for peer artifacts your
   mission consumes:
   `python C:\Users\User\SYNAPSE\harness\battleplan\bus.py read bp8 BP8-CRUX`
5. **Progress** every 5 turns - the on-target signal the orchestrator's drift
   ...
   `python C:\Users\User\SYNAPSE\harness\battleplan\bus.py post bp8 BP8-CRUX progress '{\"target\": \"T1\", \"evidence_path\": \"<path-or-none>\"}'`
...
Write `harness/notes/receipts/BP8-CRUX.json` **inside your worktree**:
`{{"leg": "BP8-CRUX", "status": "green|green_with_findings|blocked",
