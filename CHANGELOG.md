...
## Unreleased - BLOCKS: deterministic scene setups

*Branch `blocks/m5-reconciler` (`87927076`), not merged. **5,745 tests passing, 9 failing, 147 skipped** - **+85** against the M5 merge-base, all new and passing; the 9 failures are pre-existing, reproduced at merge-base with zero BLOCKS code present, and unchanged in identity and count. Windows suite, Python 3.14.2. Invariants headless on Houdini 22.0.368.*
...
**THE ENGINE (M5, `87927076`).** `synapse_apply_fixture` / `synapse_remove_fixture`, mounted on the cognitive Dispatcher seam. Six modules under `python/synapse/blocks/`: `plan.py` is the pure reconcile diff (no `hou`), `runtime.py` is the only module that touches the host, undo-grouped with post-mutation self-verification. Ruled contract, enforced by code shape rather than convention: ownership is a network box (`BLOCKS_<fixture>`, nodes flat in `/stage`, zero USD `customData` written); collisions short-circuit **before** the mutation block, so the undo group is never entered; delete candidates come only from enumerated box membership - the string `outside_names` does not appear inside `build_plan`, and a test pins that source property.

**PROVEN, NOT ASSERTED (F-1..F-5, `harness/blocks/invariants_m5.py`).** Apply on clean reproduces the committed baseline byte-exact; apply -> remove -> apply gives the same hash; apply on applied is 0 ops with the hash unchanged; an artist node outside the box keeps every authored property; a name clash refuses with the full stage signature byte-identical across the call. **Five negative controls run first** - Law 1: an instrument that has not been shown to disagree is not evidence.
...
**THE EVIDENCE HARNESS THAT PRECEDED IT (AUTORESEARCH, `1e13629f`).** Three tiers: Claude orchestrates, a local model authors and triages questions through ollama at zero API cost, hython executes probes deterministically. The model may author **questions**; only probes produce **answers**, and proposed missions pass a literal fence (only type names the evidence proved alive) plus schema validation before they can run. First campaign: 55 answers, 0 failures. It confirmed the four dead H22 literals with live successors mapped (`graft` -> `graftbranches`/`graftstages`, `usd` -> `inlineusd`, `geometryclipsequence` -> `geoclipsequence`/`valueclip`, `usdrender` -> `usdrender_rop`), caught `karmarenderproperties` **deprecated** one step before it entered the fixture, and found Houdini stamping session node IDs into composed USD as provenance - the `c2` canonicalizer rule, without which cross-session determinism was impossible.

**SEAM DRIFT, RECORDED AS FOUND.** `dispatcher.py` describes "the single entry point"; `mcp_server.py` in fact builds **four** independent Dispatcher singletons, each `is_testing=True` with its own transport closure, and the production main-thread branch is unwired - main-thread safety on this path comes downstream from the WebSocket `execute_python` handler. BLOCKS follows the established practice and claims nothing more. Recorded rather than silently adapted to.

**OPEN, RULED, NOT YET BUILT (M5b).** The baseline embeds `$HIP` and is machine-local - ruled to a `c3` canonicalizer treating environment-derived paths as session-local, plus a re-baseline. The LOP display flag is one exclusive network-wide slot, the single place where *match the definition* and *never touch artist nodes* cannot both hold - ruled to honour the fixture and report the transfer by name, with the invariant split rather than softened. A stray node dragged into the box is currently deleted - **ruled to eject instead**: the reconciler must not destroy what it never created. The committed h22 symbol table is stamped 22.0.397 on a 22.0.368 host - ruled to regenerate with a load-time build check.

**NOT DONE.** No phrase routing (M6) - both tools take a fixture name. No panel surface for the verdict line. No USD `customData` (RFC-gated). No new fixtures.
