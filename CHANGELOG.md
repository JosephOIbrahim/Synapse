...
sizing are unchanged. This patch includes the preceding panel, World Labs,
welcome and SideFX library revisions. It is a source release with no new
Windows installer. See [v5.84.1](docs/releases/v5.84.1.md).
...
**31 GUIDES THROUGH THE SAME GATE, WITH PROVENANCE (BP10-GUIDES).** `rag/ingest/guides.py` (424 lines) pulls fxhoudinimcp's guides (MIT, @ 29b6695b) into `rag/corpus/guides/*.jsonl`; `rag/quarantine/guides/` holds the five that named phantom tokens (assets, heightfields, io, model, render) with the tokens listed; `harness/notes/harvest/answer_key.json` is the empty per-token decision template and `harness/notes/harvest/ingest_report.json` the full ingest record. `harness/jev/jev_harvest.py` classifies each quarantined token (renamed node / parameter / attribute / VEX function / generic word / cannot tell) in shadow; `jev_grade.py --guard harvest` grades it against the answer key once filled. `tests/test_guides_ingest.py` (9) proves a planted phantom is quarantined. `rag/ingest/tool_map.json` regenerated (+2056/-339).

**README CORRECTED: STAGED, NOT YET SERVED.** v5.79.0 listed `rag/corpus/guides` under "What's ready". No runtime path consults it (`KnowledgeRouter` reads `documentation/_metadata/semantic_index.json` and `corpus/h22_nodes.json` only; `git grep -l -E 'scope_weights|h22_prose|corpus/guides' -- python/synapse` -> 0 files). CRUX ruled the line BROKEN as a false readiness claim on an owned surface. It now reads "Staged, not yet served", below the list, provenance kept.

**AN OUTSIDE-IN BENCHMARK (BP10-BENCH).** `harness/outside_in/run.py` runs each prompt in `prompts.jsonl` through two isolated hython arms (`arms/synapse.py`, `arms/fxhoudinimcp.py`); `verify.py` scores the resulting scene against `harness/notes/outside_in/bench_key.json`, never the transcript; a prompt-leak check (`harness/jev/ledger/bp10.leak.jsonl`) confirms neither arm sees the key. `harness/jev/jev_bench.py` is the shadow verifier. Plumbing proven on one prompt (`results.jsonl`, 2 rows); G5, the full run, is held. No comparison number is claimed.

**TWO VERIFY TOOLS (PRs #109, #110).** `harness/verify/gate_table_agreement.py`: deterministic gate-table lint, advise-only, ratcheted against `gate_table_agreement_floor.json`; 16 disagreeing rows surfaced (`harness/notes/receipts/JEV-GATELINT.json`). `harness/verify/recall_oracle.py` (667 lines): recall-quality oracle, store-copy replay with Jev relevance, answer-key validated.

**THE REFEREE'S VERDICTS, DISCLOSED.** CORPUS SOUND-WITH-NITS (the G3 probe suite collects 24, not 25, and cannot regress from a corpus the router never sees -- disclosed in the receipt); BENCH SOUND-WITH-NITS (G5 held); GUIDES BROKEN on README.md:121 only, fixed in this release, re-verdict pending; SCAFFOLD BROKEN, already on master under v5.79.0, fix-forward with four named items still open. TIDY's first receipt claimed pass on count lines reading 1 against 6 rows and 5 against 8; corrected to `green_with_findings` before merge. Two mechanical-tier legs across BP4 and BP10 have now self-graded a pass on a count predicate their own evidence contradicted; both are JEV-SCREEN calibration samples.

**HOW IT MERGED.** CORPUS and GUIDES both created `scope_weights.py`, `test_scope_weights.py` and the harvest section of `jev_grade.py` from the same base, because GUIDES declared `deps: [BP10-CORPUS]` but branched from master. GUIDES's copies are strict evolutions of CORPUS's; the merge took GUIDES's side on all three. Assembled on a throwaway `bp10/train`, full suite 9609 passed / 371 skipped / 5 xfailed / 0 failed (6m25s), then fast-forwarded onto master. A `base` that follows `deps` in `compile_wave.py` would have avoided the collision.
...
**THE WORKER ROSTER IS THE ONE THE WORKER MAY USE.** `synapse_panel.py` handed `ClaudeWorker` all 143 tool schemas although the worker's own default already filters to the allowed 103. Measured with tiktoken cl100k: 23,970 to 15,290 prompt tokens per cold call. The send path now passes `tools=None`; a test builds the worker exactly as the send path does under all five `SYNAPSE_WORKER_TOOL_MODE` values.

**ONE ROW PER TASK IN A USAGE LEDGER.** `ClaudeWorker._conversation_loop` is wrapped in try/finally; the finally appends one JSON row to `~/.synapse/usage/turns.jsonl` (provider, model, turns, tool calls with per-tool errors, `UsageSink` fields, per-stream wall ms, stop reason, outcome in completed/stopped/error/cap_hit). The append is fenced so a disk error can never reach the artist. This is the answer key the router work in the 2026-09-21 scout report is graded against; nothing reads it yet. Override path: `SYNAPSE_USAGE_LEDGER`.

**THE FENCED JEV DOOR EXISTS, WITH NO CALLERS.** `python/synapse/jev/adapter.py::judge()` implements the six conditions of the amended invariant 5: 800 ms wall-clock timeout on a daemon thread, None on any failure, `SYNAPSE_JEV` off by default and short-circuiting before any import, key resolved once and never logged, ledger under `~/.synapse/jev/`, refusal on the Houdini main thread, shadow mode recorded on every row. 22 tests, all with a mocked client. Nothing under `python/synapse` imports `harness/jev` (`tests/test_jev_product_boundary.py`).

**THE LEGACY CHAT SURFACE IS RETIRED (step A).** `chat_panel.py`, `quick_actions.py`, `synapse_chat.pypanel` and the `route_chat` WS command and handler are deleted; `tests/test_single_pypanel.py` pins that exactly one pypanel is installable. Deleting the legacy loader turned the `hot_reload_gated` check honestly green. Tier 2/3 of `TieredRouter` stay until the code-first prefilter lands (RULINGS.md, ruling 3). The panel rhythm ratchet moved 13 to 9.

**THE HARNESS: JEV ROUTED THIS RELEASE.** Ten legs went through the shipped ROUTE and TEAM guards plus two new pick Choices (builder roster, verifier roster), every call ledgered under `harness/jev/ledger/bp9.*`; each leg built in an isolated worktree and was attacked by a crucible before merge (`harness/notes/bp9/`). Rails gained a `none` tier: probe legs run inside the orchestrator with no model, the command travels by file so quotes survive PowerShell 5.1, and a probe that prints no ACCEPT line fails. The 70M wave cap is enforced cost-weighted (input x1, cache create x1.25, cache read x0.1, output x5); BP8 recomputes to 20,118,147 against 79.8M on the rails basis. Claude Code hooks: `guard-edit-targets.py` fails closed, guards `~/houdini22.0`, and fences worktree agents out of the main checkout while leaving `~/.claude` writable; a hook-fire ledger at `~/.synapse/hooks.jsonl`; Stop and TaskCompleted exit 2 on an unresolved cook error, cleared only by a successful cook or an explicit "cook errors resolved" / "resolved:" phrase; the `codebase-transition` skill CLAUDE.md names now exists. H22 doc-intel workflows and the probe runner read the live build stamp instead of 22.0.368 literals; a `checks.py` verb is RED until the packaged Solaris knowledge is re-authored on 22.0.400.
...
**THE REPORT BEHIND THE RULING.** `docs/reviews/jev-opportunities-2026-09-20.md`: live TypeSafe docs read first, then one dynamic workflow of 30 agents (8 seam cartographers, triage, a crucible attack per finalist, a docs-fit reshape per survivor, a completeness critic). 38 candidate judgment points, 94 rejected as not Jev-shaped, 12 finalists, 8 survived attack, 4 broken with pure-code survivors. Five build-time items are shippable under the invariant as amended (gate-table lint, recall-quality oracle, CTO SYNTH triage, stale-leg classifier in shadow, scout shadow reranker); three product items are now unblocked (loop-drift nudge, build-shape hint, worker denial as a consent card).

**THE SHIPPED GUARDS, GRADED.** `python harness/jev/jev_grade.py` over n=8: SCREEN's current policy has never said CLEAR and said REFEREE on the one BROKEN leg; the `flag-on-contradiction` candidate is the only one that catches it. ROUTE on the one live `tier: auto` wave rounded up 6/6 at confidence 0.16 to 0.57. Mission `note` fields open with a literal "Tier: mechanical (Haiku 4.5)" sentence, so the shadow agreement could have been leakage; a re-run with every tier and model word scrubbed still agreed 12/12 (`harness/jev/ledger/leakprobe-bp4.shadow.route.jsonl`, `leakprobe-bp8.shadow.route.jsonl`). `harness/jev/tests/` is still outside `pyproject` testpaths and `ci.yml`; recorded, not fixed here.

## v5.77.0 - Turn two answers, and the harness picks its own graph

*Product change in `python/synapse/routing/router.py`, `python/synapse/server/handlers.py` and `python/synapse/panel/chat_panel.py`. No new installer: the 5.75.2 Setup.exe does not carry these fixes. Full notes: `docs/releases/v5.77.0.md`.*

**THE ROUTER'S TIMEOUTS EXISTED AND WERE NEVER USED.** `RoutingConfig.tier2_timeout` (5 s) and `tier3_timeout` (15 s) were defined at `router.py:124-125` and passed to nothing: the `guarded_create` LLM calls ran unbounded, so a slow model turn held the handler until the 30 s slow-op kill, which emits no reply at all, and `ws_bridge.py` drops any reply that lacks `response`/`tier`. Traced by BP7 scout H4 (`harness/battleplan/notes/BP7_VERDICT.md`), fixed by BP8-TIMEOUTS: the timeouts now reach the SDK through the LLM pool, a timed-out tier records a failure metric and falls through the cascade, and `_handle_route_chat` bounds the whole `route()` call so the turn always answers with a well-formed `{response, tier}`. Six tests; the referee removed the timeout argument and watched the sleep test redden.

**THE PANEL HAD NO WAY TO RECOVER FROM SILENCE.** The waiting state and typing indicator were cleared in exactly two places, both of which required a message to arrive. BP8-WATCHDOG arms a single-shot `QTimer` on send (`WATCHDOG_TIMEOUT_MS = 35000`, the server's budget plus a margin, a named constant) that clears the state and appends one system line; connection error and disconnect now clear the same state through one shared helper. Headless tests plus a real-QTimer probe on hython 22.0.400. **One acceptance row is UNKNOWN by rule** - the live H22 panel showing the recovery line - and stays UNKNOWN until a human clicks it.

**BOTH LEGS SOUND-WITH-NITS BY AN ADVERSARIAL REFEREE** (`harness/battleplan/notes/BP8-CRUX_verdicts.md`): every headless row re-run in fresh clones, a binding proof that the new tests fail against the old package, 18 self-authored mutations of which 13 reddened the named test. One nit fixed before merge: the handler's off-thread safety comment claimed `route()` touches no `hou.*` API; it reaches `SynapseMemory.search`, which is `@_on_memory_main` and marshals itself. True conclusion, false premise, now a true premise.

**THE HARNESS GAINED THE JEV HELM** (`harness/jev/`, card `harness/battleplan/notes/JEV_HELM.md`). Four guards beside ROUTE and SCREEN: JEV-SHAPE picks a wave-graph template from an objective (live; refused a bundled objective at 0.54, correctly); JEV-TEAM caps subagents per leg, rounding down on doubt (live; zero on all twelve real legs judged); JEV-EDGE inserts one bounded repair leg on a FLAG receipt (shadow); JEV-DRIFT judges whether a leg is looping (shadow only by construction - a test fails if the file ever gains a bus writer). BP8 was the first wave shaped by the helm. **SCREEN's count comparison moved into code** after Jev 1.13's model card stated it "does not count reliably": the BP4-RULINGS receipt (22 expected, 21 reported, CRUX BROKEN) that Jev had scored 0.25 is now caught exactly. `jev_grade.py` replays every ledgered judgment under candidate policies against CRUX verdicts with zero new calls. 55 biting tests, up from 14. Measured BP8 spend from transcripts: 22.2M + 26.7M builders, 30.9M referee; the 70M ceiling halted TIDY, as designed.

## v5.76.1 - The harness learns to judge before it spends

*Harness only; nothing under `panel/` or `synapse/` changed. Notes: `docs/releases/v5.76.1.md`. (Entry backfilled in 5.77.0; the release itself did not add one.)*

**JEV JOINS THE BATTLEPLAN HARNESS AS TYPED GUARD NODES.** TypeSafe's System One model returns calibrated judgments, not text. JEV-ROUTE resolves a mission authored `"tier": "auto"` to a `rails_exec.json` tier name at compile time from three judgments and one plain-code policy that rounds up on doubt (8/8 with hand-authored tiers on BP4). JEV-SCREEN judges each builder receipt per acceptance row and writes one line into the referee's brief, setting where it starts reading; it never removes the referee. Every call ledgered under `harness/jev/ledger/`; every failure falls closed to the pre-Jev behaviour; `SYNAPSE_JEV=off` is the kill switch. Fourteen biting tests. `v5.76.0` stayed a tag that never became a release because its CI was red on a README tag line and a phrasing-pinned doc test; both fixed here.
