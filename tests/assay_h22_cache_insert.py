...
Then it exercises the FAILURE path (closeout PLAN Stop 3): a fault injected right after
``createNode`` inside the same REAL ``hou.undos.group`` records what survives (the partial
network -- grouping is not rollback) and what one ``performUndo`` then does to it. "One Ctrl+Z
reverses it" is a success-path claim; items 7-8 are the failure-path evidence next to it.
...
Artifact contract (contract-assay keys plus ``certifies``):
    {schema: "cache_h22_insert_assay/v1", houdini_version, platform, command, exit_status,
     ...
     certifies: {assay_script_sha256, handlers_cache_sha256, git_head_at_run}}
    ``certifies`` ties the receipt to the code it ran against: the sha256 of this script and of
    ``handlers_cache.py`` are the load-bearing check (a receipt cannot carry the sha of the commit
    it ships in); ``git_head_at_run`` is the worktree HEAD at run time, informational.
Receipt hygiene: the receipt NEVER carries an absolute local path. The ``$HIP``-expanded cache
    path is used for the on-disk existence check only (a receipt leaked one once -- public repo).
...
class _Delegate:
    """Attribute-delegating wrapper around a REAL hou node (fault-injection base)."""
...
class _RaiseOnSetInput(_Delegate):
    """The node handed back by createNode: its very next call in insert_cache_core is setInput."""
    def setInput(self, *args, **kwargs):
        raise RuntimeError("assay-injected failure: first call after createNode inside the undo group")


class _FaultParent(_Delegate):
    def createNode(self, *args, **kwargs):
        return _RaiseOnSetInput(self._real.createNode(*args, **kwargs))  # REAL node, REAL scene


class _FaultAfterCreate(_Delegate):
    """Real source node whose parent() creates real nodes that raise on the call after creation."""
    def parent(self):
        return _FaultParent(self._real.parent())
...
def _run_all() -> list:
    ...
    try:
        ...
        # Receipt hygiene: the $HIP-expanded path is an ABSOLUTE local path. It is used for the
        # on-disk existence check ONLY; the receipt carries the raw $HIP-relative parm string,
        # the cache dir's basename and a boolean -- never parm("file").eval().
        item5 = _new_item(5, "assert no cache files exist on disk (insert never writes/cooks)")
        file_parm = cache_node.parm("file")
        file_raw = file_parm.rawValue()                   # e.g. $HIP/cache/src_box/src_box.$F4.bgeo.sc
        resolved_dir = os.path.dirname(file_parm.eval())  # absolute -- NEVER written to the receipt
        cache_dir_basename = os.path.basename(resolved_dir)
        cache_dir_exists = os.path.isdir(resolved_dir)
        if not cache_dir_exists:
            _record_pass(item5, f"file_raw={file_raw!r} cache_dir_basename={cache_dir_basename!r} "
                                f"cache_dir_exists={cache_dir_exists!r} (nothing written), "
                                f"cooked={ins['cooked']!r} path_written={ins['path_written']!r}")
        else:
            files = sorted(os.listdir(resolved_dir))      # basenames only
            if not files:
                _record_pass(item5, f"file_raw={file_raw!r} cache_dir_basename={cache_dir_basename!r} "
                                    f"cache_dir_exists=True but empty (nothing written), "
                                    f"cooked={ins['cooked']!r} path_written={ins['path_written']!r}")
            else:
                _record_fail(item5, f"cache dir {cache_dir_basename!r} contains files after insert "
                                    f"(should be none -- insert must never write): {files!r}")
        ...
        # --- items 7-8: the FAILURE path (closeout PLAN Stop 3: grouping is not rollback) ---
        # A raise inside hou.undos.group after createNode must leave a partial network: the group
        # closes, nothing unwinds. Fault injection keeps everything real -- the REAL insert core,
        # the REAL source node, the REAL hou.undos.group, a REAL createNode -- and only wraps the
        # node createNode hands back so its very next call (setInput) raises.
        item7 = _new_item(7, "FAILURE PATH: a raise inside hou.undos.group after createNode leaves "
                             "a partial network in the scene (grouping is not rollback)")
        resp2 = hc.assess_cache_core(
            source, node_path=source.path(), node_type=source.type().name(), machine=machine,
            context="sop", is_solver_result=True, frame_range=(1001, 1240),
            issued_decision_store=store,
        )
        decision_id2 = resp2["decision"]["decision_id"]
        pre7_children = sorted(c.name() for c in container.children())
        pre7_downstream = downstream.inputs()[0].path() if downstream.inputs() else None
        pre7_undo_depth = len(hou.undos.undoLabels())
        raised = None
        try:
            hc.insert_cache_core(
                decision_id=decision_id2,
                resolve_source_node=lambda: _FaultAfterCreate(source),
                store=store,
                undo_context_factory=lambda: hou.undos.group("SYNAPSE insert_cache (forced failure)"),
            )
        except Exception as e:  # noqa: BLE001 -- the injected fault, propagated out of the group
            raised = f"{type(e).__name__}: {e}"
        post7_children = sorted(c.name() for c in container.children())
        survivors = [c for c in container.children() if c.name() not in pre7_children]
        survivor_desc = [(c.name(), c.type().name(), [i.path() for i in c.inputs()])
                         for c in survivors]
        post7_downstream = downstream.inputs()[0].path() if downstream.inputs() else None
        if raised is None:
            _record_fail(item7, "fault injection did not fire -- the failure path was NOT "
                                f"exercised; children={post7_children}")
        elif (len(survivors) == 1 and survivors[0].type().name().startswith("filecache")
              and post7_downstream == pre7_downstream):
            _record_pass(item7, f"raised={raised!r}; partial network SURVIVED the exception: "
                                f"survivors={survivor_desc!r} (unwired File Cache); downstream "
                                f"still wired to {post7_downstream!r}; undo depth "
                                f"{pre7_undo_depth}->{len(hou.undos.undoLabels())} -- the group "
                                f"closed, nothing unwound")
        else:
            _record_fail(item7, f"raised={raised!r} but the scene is not the expected partial "
                                f"state: survivors={survivor_desc!r} downstream="
                                f"{post7_downstream!r} (pre={pre7_downstream!r}) "
                                f"children={post7_children}")

        item8 = _new_item(8, "FAILURE PATH: one hou.undos.performUndo() removes the orphaned "
                             "partial network (the artist must undo it deliberately)")
        if raised is None:
            item8.detail = "not exercised: item 7 did not raise"   # status stays not_run
        else:
            top_label8 = list(hou.undos.undoLabels())[:1]
            hou.undos.performUndo()
            post8_children = sorted(c.name() for c in container.children())
            survivors_gone = all(hou.node(container.path() + "/" + n) is None
                                 for n, _t, _i in survivor_desc)
            downstream8 = downstream.inputs()[0].path() if downstream.inputs() else None
            if (survivors_gone and post8_children == pre7_children
                    and downstream8 == pre7_downstream):
                _record_pass(item8, f"undo label={top_label8!r}; one performUndo removed "
                                    f"{[n for n, _t, _i in survivor_desc]!r}; children="
                                    f"{post8_children} (== pre-failure {pre7_children}); "
                                    f"downstream wired to {downstream8!r}")
            else:
                _record_fail(item8, f"one performUndo did NOT clean the partial network: "
                                    f"survivors_gone={survivors_gone!r} children={post8_children} "
                                    f"pre={pre7_children} downstream={downstream8!r}")
    except Exception:
...
def _certification() -> dict:
    """What this receipt certifies. Content hashes are the load-bearing check; git HEAD is the
    worktree commit at run time (the amend that ships the receipt necessarily post-dates it)."""
    import subprocess
    ...
    except Exception:  # noqa: BLE001 -- git absent is not an assay failure
        head = None
    return {
        "assay_script_sha256": _sha256("tests/assay_h22_cache_insert.py"),
        "handlers_cache_sha256": _sha256("python/synapse/server/handlers_cache.py"),
