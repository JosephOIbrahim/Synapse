...
**violation** state to the green (``CONIFEROUS``) token would pass CI undetected --
...
def _count_ok_soft(fn):
    """(total CONIFEROUS returns, CONIFEROUS returns inside a `violations == 0` If)."""
    total = sum(1 for n in ast.walk(fn) if _returns_attr(n, "CONIFEROUS"))
    guarded = 0
    for node in ast.walk(fn):
        if isinstance(node, ast.If) and _is_all_clear_guard(node.test):
            for stmt in node.body:
                if _returns_attr(stmt, "CONIFEROUS"):
                    guarded += 1
    return total, guarded


def test_fidelity_color_greens_only_all_clear():
    """``_fidelity_color``: every green (CONIFEROUS) return is gated by a
    ...
    assert total >= 1, "expected an CONIFEROUS (green) branch — the all-clear state"
    assert total == guarded, (
        "CONIFEROUS (green) reachable without a `violations == 0` guard -- "
...
def test_fidelity_color_no_data_returns_slate_not_green():
    ...
    for r in guard_returns:
        assert _attr_name(r.value) != "CONIFEROUS", (
            "no-data guard returns green CONIFEROUS -- a lying 100%"
        )
...
def test_set_integrity_delegates_green_to_the_guard():
    """``set_integrity`` must NOT name CONIFEROUS directly — the color decision is
    ...
    for node in ast.walk(fn):
        assert _attr_name(node) != "CONIFEROUS", (
            "set_integrity references CONIFEROUS directly -- green must route "
