...
**Off-ledger work admitted under the freeze (2026-07-21):** the marshal-deadlock sprint belongs to no leg. It merged to master as **`73304bf`** with Joe's explicit authorization — logged as drift **D4** and amendment **A2** (§9). No leg's state changed as a result: Leg 1 stays IN-FLIGHT (the recording is Joe's), Leg 6 stays PENDING. What it changes is the ground the remaining legs stand on — the **main-thread self-deadlock class is eliminated and lint-enforced**, so a permanent UI lock during the Leg 1 recording or the Leg 6 stranger-run is no longer the risk it was. It is not a freeze-proofing: on the main thread a render still holds the UI for its duration — bounded now, and it **completes**. Cancel during a long operation still does not work — see D4.
...
- [x] **Refresh `DEMO_SCRIPT.md` first** — the current file is dated 2026-02-08 and describes a 23-tool / 376-test product (at the time of this rewrite: 115 tools, ~4,571 tests; suite is **4,642** as of `73304bf` — see §8 for the live count). Rewrite to H22 reality without redesigning the demo's shape. `run_demo_builder.py` is retired from the critical path (H21-era, machine-specific OneDrive paths — `run_demo_builder.py:8-11`); the demo entry is `demo/synapse_demo.hip`. *(Done 2026-07-19: 3 truth-mappers grounded the rewrite — 115 tools per the DOC-1 method, real artist flow incl. footer-Connect + OCIO + task 0.5, bounded-render Act 3 with the Indie in-process truth; 19 stale claims dropped, all listed in the workflow record.)*
...
- **Suite baseline, stated honestly:** ratchet floor **4275 / 0 / 87** (`harness/verify/suite_baseline.json`); live local **4,642 passed / 0 failed / 100 skipped** as of `73304bf` — one full headless `pytest tests/` run, no live bridge. The 4 known `test_bridge_endpoint` live-sidecar isolation failures did not fire in that configuration and are **not thereby proven fixed** — they remain pre-existing tracked debt that appears when a live sidecar is present (CI green; CI lacks one). Gates exclude those 4; fixing them is in-scope fix-forward, weakening them is not, and release week must not grow the set.
...
| A2 | 2026-07-21 | none (out-of-list work) | Marshal-deadlock sprint admitted to master under the freeze: merge `73304bf`, version line declared **v5.33.0**. No leg's scope, tasks, or Done-when changed; no leg added. | The freeze's own §0.3 permits fix-forward, and the defect is a **permanent, unrecoverable UI lock reachable from the panel and from `/mcp`** — it would have taken the Leg 1 recording with it. The sprint held itself on branch (gate_log R3) precisely so this call stayed Joe's. See **D4** for the verified anchors. | **Joe (explicit, 2026-07-21).** Order irregularity recorded honestly: the authorization followed the work, so F3 (amendment commits *before* the amended work runs) did not hold here. |
...
| D4 | 2026-07-21 | none (out-of-list work) | **Work executed and merged outside every leg's task list.** A marshal-deadlock sprint ran under CTO authority and merged to master as `73304bf` (branch `fix/marshal-deadlock-class`, fix commit `607fc73`). It appears in no leg of this document, so §0.3 (task-lists-plus-fix-forward) and §0.5 (Joe-only scope amendments) both bind. The sprint logged this itself before merging — `docs/sprint_freeze/gate_log.md` **DRIFT-01** + ruling **R3** held all work on the branch and declined to self-authorize the merge; `done_signoff.md` §"Not signed by me" repeats the refusal. **Verified anchors:** defect proven from vendor source (`hdefereval.py::_queueDeferred`, `_condition.wait()` with no thread check — a main-thread caller parks forever waiting on itself, plus a `_last_result` module-global that can swap results between concurrent blocking marshals); nine call sites bypassing `server/main_thread.py::run_on_main` deleted, three of them calling `hdefereval.executeInMainThread`, **which does not exist on H22.0.368** and was failing silently; live-verified on H22.0.368 PID 64396 (identity-probed first) — `run_on_main` from main returns inline in 0.014ms, `houdini_capture_viewport` over `/mcp` (a confirmed pre-fix deadlock path) returned twice with `Responding=True` throughout, a deliberate 6s main-thread hold produced exact telemetry attribution (`fast_path_2`, 6.0004s vs 5.0s budget) with 0 violations and the next turn served 22µs later, and a ~25-call soak (5 concurrent read-only, 4 concurrent mutating) returned each call's own payload with 0 violations / 0 stack dumps / 0 freezes. Suite **4,642 / 0 / 100** against floor 4,275; 7 of 8 DoD items GREEN. | **None proposed — the hold was lifted, not amended.** Joe authorized the merge explicitly on 2026-07-21; recorded as **A2**. Logged rather than absorbed because the authorization came *after* the work, so F3's "amendment commits before the amended work runs" did not hold in order — the ledger is honest about that rather than backdated. **Sign-off item D3 remains PARTIAL, not green** (that is `done_signoff.md`'s DoD numbering — unrelated to drift D3 above)**:** `websocket.py:471` handles messages serially, so `cancel` / `emergency_stop` / `status` queue behind the very handler being cancelled — cancel does **not** work during a long operation. That is the **#1 follow-up** and is not in this week's scope. |
...
- **2026-07-21** · No leg (off-ledger, Joe-authorized) · **Marshal-deadlock sprint merged to master: `73304bf`, version line v5.33.0.** Nine call sites that bypassed `server/main_thread.py::run_on_main` deleted — three of them calling `hdefereval.executeInMainThread`, a function that does not exist on H22.0.368 and was failing silently. The fix was removing the bypasses, not building a new system; `run_on_main` was already immune. A line-scoped source lint keeps the unsafe primitive out permanently. Live-verified on graphical **H22.0.368** (PID 64396, identity-probed before any repro): main-thread `run_on_main` returns inline in 0.014ms where it previously hung forever; a former `/mcp` deadlock path returned twice with the UI Responding throughout; a ~25-call concurrent soak showed 0 violations, 0 stack dumps, 0 freezes, and no result swapping. Suite **4,642 passed / 0 failed / 100 skipped** (floor 4,275), two tests re-anchored and both proven able to fail, none weakened. **7 of 8 sign-off items GREEN; item D3 PARTIAL** (`done_signoff.md` numbering, not this doc's drift IDs) — cancel still cannot be delivered during a long operation (`websocket.py:471` serial dispatch), the #1 follow-up, not fixed and not scoped this week. State changes: none to any leg; drift **D4** + amendment **A2** logged. Blockers: unchanged — Leg 1's recording and Leg 6's stranger-run are Joe's.
  *Anchor note:* `73304bf` is the merge commit on master; it does **not** itself carry the version bump — `VERSION`/`pyproject`/`__init__`/CHANGELOG were still on `5.32.1` at that commit and are being reconciled to `5.33.0` in a follow-on commit. **No `v5.33.0` tag exists**, and none should yet: tagging is a §8 human gate reserved to Leg 7. Record this as a version *line* declared, not a release published.
