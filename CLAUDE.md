...
> **⚠ R18+R19 / RSI loop F — outcome veto is honest but DORMANT.** Frequency alone cannot represent failure, so `MOERouter.record_outcome(fingerprint, success)` is the outcome channel: any recorded failure vetoes promotion of that fingerprint and evicts an existing session entry for it. Entries carry `outcome_confirmed` (tuple index 3) — `True` only when a success was actually observed; a later recorded success upgrades it `False → True` (R19b), because promotion necessarily fires before any outcome exists.
>
> **`success` must be a real `bool`.** R19a: anything else raises `TypeError` and records nothing. Under the original truthiness check, `record_outcome(fp, "FAIL")` — the obvious way to miswire a producer — incremented the **success** tally and manufactured positive evidence for a route that had just failed. If an outcome is genuinely unknown, **do not call**.
>
> **Dormant, not merely unfed.** Nothing calls `record_outcome()`, *and* the only non-test caller of `MOERouter.route()` is `panel/tool_filter.py:245` inside `filter_tools()` — which has **zero references anywhere in the repo**. The promotion path executes zero times in production, so the open question is wire-or-delete, not wire. `_session_fast_paths` remains in-memory only — deliberately, per `harness/rsi/REGISTRY.json` loop F: persisting a promotion table before its signal is honest is the hazard, not the feature.
