...
> **Target:** Houdini 22.0.400 (dual-build with H21 artifacts) · SYNAPSE v5.50.0 · Python 3.13 · 129 MCP tools registered
...
- **`/synapse` (live WS) → `server.handlers` directly:** RBAC-gated, main-thread-marshalled (`run_on_main`), 30s slow-op timeout, undo-wrapped only **partially** (⚠ drift, verified 2026-07-10; updated 2026-08-15 W5-UNDO: usd/material/cops/batch/execute handlers wrap in `hou.undos.group`, and as of W5-UNDO the `handlers_node.py` create/connect/delete handlers wrap too — **grouping only, not rollback** (see next paragraph). The fourth handler the earlier note named, `set_parm`, is UNWRAPPED still and actually lives in `handlers.py:_handle_set_parm`, **not** `handlers_node.py` — so of the four handlers this note tracks, exactly that one hole remains, pending a follow-up leg on that out-of-`touches` surface. Scope caveat: other live mutating handlers outside that quartet are not audited here — `set_keyframe` (`handlers_render.py:_handle_set_keyframe`) is at least one more unwrapped case. And `server/integrity_envelope.py`'s honesty docstring (~L19-28) still lists these three node handlers as unwrapped and brands this §1 claim "doc drift" — that cross-reference is now stale for create/connect/delete and is tracked for the same follow-up leg). **Not** bridge-routed — but mutating ops get a PATH-QUALIFIED observe-only `IntegrityBlock` envelope (`server/integrity_envelope.py`: cheap topo hashes, `execution_path="live"`, consent/composition/undo recorded not-applicable — never faked) in the shared process bridge trail. Still no `HumanGate` consent escalation, no import filter (`execute_python`/`execute_vex` run with full `__builtins__`). The *RBAC-guarded* path.
