...
**Trap:** passing an `xn__…` punycode name to `set_usd_attribute` **silently no-ops** — the handler tries `prim.GetAttribute("xn__inputsradius_mva")` (invalid) then `prim.GetRelationship(...)` (also invalid for a punycode name), so it writes nothing and raises no error. Conversely, `node.parm("inputs:radius")` returns `None`. Always match the name space to the API: raw on the prim, punycode on the parm.

**Relationships are handled (H22 / W.5-H22-karmarels):** some USD properties are **relationships**, not attributes — `light:filters`, `collection:lightLink:includes`/`excludes`, `collection:shadowLink:includes`/`excludes`, `proxyPrim` on lights; `camera`, `products`, `orderedVars`, `husk:orderedImageFilters` on RenderSettings/RenderProduct; `collection:filterLink:*` on a light filter. `prim.GetAttribute(<rel>)` is **invalid** for these even when populated, so `get_usd_attribute`/`set_usd_attribute` fall back to `GetRelationship`: read returns the targets as a path-string list with `property_kind: "relationship"`; write authors via `SetTargets` (a list of prim paths), creating the relationship only when it is schema-defined/authored or the value is unambiguously a list of `/`-rooted paths. Note (N-3, 22.0.368): **zero `karma:*` properties are relationships** — all `karma:global:*`/`karma:object:*` plus `husk:default_delegate`/`husk:scene_lights`/`husk:scene_materials` are plain attributes; the only husk-namespaced relationship is `husk:orderedImageFilters`.
