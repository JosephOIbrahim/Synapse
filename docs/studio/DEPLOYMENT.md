...
| `SYNAPSE_WORKER_TOOL_MODE` | Autonomous-worker tool policy: `strict` / `standard` / `unrestricted` / `demo` (bp5: read tools + `synapse_run_recipe` only). Absent = default; an explicit unknown value or a value that conflicts with the profile resolves to `strict` (fail closed, bp5 AUTHORITY -- ruling pending, see docs/solaris_v3/INTEGRATION.md) | `standard` | `panel/worker_policy.py` | Studio: standard or strict |
