...
**Governing gate:** merge-to-main, **per wave, human** (`docs/H22_AGENT_HARNESS.md:55` gate registry — "Merge-to-main | no file — always human, per commit" — whose §2 header states that table "renders blueprint §8 made mechanical"; `docs/H22_AGENT_HARNESS.md:73` port-wave exit = "merge the worktree"). No agent — gatewarden, forge, assayer, crucible — merges. The workflow returns `MERGE_READY`; the human merges. `[The blueprint text itself is not in the repo (per the [UNVERIFIED] note below); this citation is to the in-repo playbook that mechanizes blueprint §8, not to the blueprint.]`
**Relay leg:** **Leg 3** (opened by `harness/state/drop.json`, which arms Legs 2–3 — `docs/H22_AGENT_HARNESS.md:53`), designated **first priority** by dispatch. `[UNVERIFIED — the SYNAPSE_H22_GAP_BLUEPRINT v2.0 source text is not in the repo; only the executor playbook docs/H22_AGENT_HARNESS.md is. The precise "Leg 3 = first-priority" ordering is asserted by dispatch, not confirmable against the blueprint locally. Verify against the blueprint doc.]`
...
**115 ≠ 104.** The delta of 11 is unexplained by any derived subset above; the most likely reading is that **104 is a stale count from an earlier registry revision**. `[UNVERIFIED — where 104 originates; it matches none of {115 registry, 123 listed, 113 = 115−undo/redo, 111 = 115−4 OD-3}. Re-derive any time with: python -c "import importlib.util as u; s=u.spec_from_file_location('r','python/synapse/mcp/_tool_registry.py'); m=u.module_from_spec(s); s.loader.exec_module(m); print(len(m.TOOL_DEFS))".]` CLAUDE.md's own header ("115 MCP tools registered") agrees with the derived 115.
