...
**Third source checked, same null result:** `sidefx.com/docs/houdini/hapi/` (the Houdini Engine API — embedding Houdini assets in external applications) is a stable API reference with no changelog content at all; it confirms zero mentions of any of the four outstanding items. Three independent official SideFX doc surfaces (HOM/user docs, HDK, HAPI) now agree: nothing supports "CORE-MATH," "Vulkan-based OpenCL interoperability," `walkonsurface`, or `curveanimate` as real. Treated as UNVERIFIED, trending toward almost-certainly-fabricated, but held short of REFUTED per this project's own discipline of never asserting absence-of-evidence as proof.
