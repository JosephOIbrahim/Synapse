...
**Third source checked, same null result:** `sidefx.com/docs/houdini/hapi/` (the Houdini Engine API — embedding Houdini assets in external applications) is a stable API reference; the artist-facing page fetched carries no changelog content, and it confirms zero mentions of any of the four outstanding items. *(Precision correction, 2026-07-16 evening: the broader `hengine` doc set DOES carry a per-major changelog at `_h_a_p_i__migration.html` — a later RETINA cross-reference pass fetched it, and it too contains **none** of the four items, making it a FOURTH independent official null surface.)* Independent official SideFX doc surfaces (HOM/user docs, HDK, HAPI artist page, hengine changelog) now agree: nothing supports "CORE-MATH," "Vulkan-based OpenCL interoperability," `walkonsurface`, or `curveanimate` as real. Treated as UNVERIFIED, trending toward almost-certainly-fabricated, but held short of REFUTED per this project's own discipline of never asserting absence-of-evidence as proof.
