...
Keep one durable store owner. Comparison records and admission state extend that owner through versioned tables or interfaces; they must not create a second conflicting job registry. Render, comparison and final submission pass through the existing consent and ownership checks on every supported transport. [Current plan schema](../../python/synapse/farm/models.py#L131), [request service](../../python/synapse/farm/service.py#L25).
...
This revision combines the 7 September blueprint, the qualified implementation record, the [TRIAGE concept and source findings](TRIAGE_FARM_MIGRATION.md), and independent artist, architecture and TOPs reviews; the blueprint, record and reviews are local research notes, not in tree.

The supplied panel's actual path is `$HOUDINI_USER_PREF_DIR/python_panels/tops_farm.pypanel`. It loads `G:/KARMA_TRIAGE_FARM/panel/tops_farm_panel.py`. The July prototype was documented against 22.0.368 and was inspected statically. Its existence is not a new render or farm qualification.
