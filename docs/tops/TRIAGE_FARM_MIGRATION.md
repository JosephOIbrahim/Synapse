...
The installed entry is `$HOUDINI_USER_PREF_DIR/python_panels/tops_farm.pypanel` (line 8), with interface name `tops_farm` and label **Triage Farm**. Its loader uses `TRIAGE_FARM_ROOT`, defaulting to `G:/KARMA_TRIAGE_FARM`, then imports `panel/tops_farm_panel.py`. No process, user or machine override was observed during this inspection.

The actual implementation is the local `G:/KARMA_TRIAGE_FARM` project (its `README.md`; not in tree). Its saved graph is `G:/KARMA_TRIAGE_FARM/out/triage_farm.hip`. This separate project explains why the prototype was absent from the tracked SYNAPSE source reviewed on 7 September.
