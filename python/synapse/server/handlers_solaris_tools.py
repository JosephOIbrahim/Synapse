...
SR1 M5 (CTO Ruling 13 discharged): `solaris_import_megascans` is no longer
gated. F9 (createNode into the locked `componentgeometry` HDA -- now built in
its writable `sopnet/geo` subnet) and F3 (the orphaned material Reference LOP
-- now wired to input 1 of the component material) are repaired and proven by
live 22.0.368 oracles in `tests/solaris/`. All five tools are registered and
reachable; pinned by `tests/test_solaris_tool_registration.py`.
"""
...
class SolarisToolsMixin:
    ...
    def _handle_solaris_import_megascans(self, payload: Dict) -> Dict[str, Any]:
        ...
        Registered in SR1 M5 (Ruling 13 gate discharged: F9 + F3 repaired and
        live-proven on 22.0.368).
        """
