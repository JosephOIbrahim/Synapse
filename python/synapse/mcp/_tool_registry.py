    ...
    ("synapse_solaris_assemble_chain", "solaris_assemble_chain", _identity,
     ...
     {"type": "object", "properties": {
         ...
         "layout": {"type": "string", "enum": ["vertical", "horizontal"],
                    "description": "Layout for nodes this assembly wires. Default vertical. A no-op does not rearrange an already wired network."},
    ...
    ("synapse_solaris_build_graph", "solaris_build_graph", _identity,
     ...
     {"type": "object", "properties": {
         ...
         "nodes": {"type": "array", "items": {"type": "object", "properties": {
             ...
             "existing": {"type": "boolean", "description": "Reference an artist-owned node for wiring only; leave its parameters, position and display flag intact."},
             "path": {"type": "string", "description": "Existing node path, absolute or relative to parent. Use path or name when existing is true."},
         }, "required": ["id"]}, "description": "Nodes to create or reuse by name and type (type required), or existing:true references identified by name/path."},
...
TOOLS_LIST_CACHE: list[dict] = sorted(TOOL_JSON.values(), key=lambda t: t["name"])

# Sorted tool names for quick lookup
TOOL_NAMES: list[str] = sorted(TOOL_DISPATCH.keys())


# =========================================================================
# Public payload builder aliases (for import by other modules)
# =========================================================================
...
network_explain_payload = _network_explain_payload
next_call_id = _next_call_id
dumps_str = _dumps_str
