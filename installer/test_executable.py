...
if __name__ == "__main__":
    ...
    parser.add_argument("--expected-version", default=(Path(__file__).resolve().parents[1] / "VERSION").read_text(encoding="utf-8-sig").strip(),
                        help="Version the newly built Setup must install; defaults to this source checkout.")
    ...
    check("Upgrade switches active payload", code == 0 and current.get("version") == args.expected_version and current.get("active") != old["active"], exit_code=code)
